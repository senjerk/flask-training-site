...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Congratulations! You now a registered user.')
        return redirect(url_for('login'))
...
def unfollow(username):
    ...
    db.session.commit()
    flash('You are unfollowed {}.'.format(username))
    return redirect(url_for('user', username=username))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('reset_password_request.html', title='Reset', form=form)
